...
export default function FacultyDashboard() {
  ...
  const [facultyDetails, setFacultyDetails] = useState<{ _id: string; name: string; email: string; department: string } | null>(null);
  const [students, setStudents] = useState<Student[] | null>(null);

  useEffect(() => {
    async function fetchDetails(facultyId: string) {
      try {
  ...
  const handleButton = async (usn: string, subCode: string, status: string) => {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-200 p-6">
      ...
      <div className="flex space-x-4 mb-6">
        <button
          className={`px-4 py-2 rounded-lg font-semibold ${activeTab === "main" ? "bg-blue-600 text-white" : "bg-gray-700 hover:bg-gray-600"}`}
          onClick={() => setActiveTab("main")}
        >
          Main
        </button>
        <button
          className={`px-4 py-2 rounded-lg font-semibold ${activeTab === "passed" ? "bg-blue-600 text-white" : "bg-gray-700 hover:bg-gray-600"}`}
          onClick={() => setActiveTab("passed")}
        >
          Passed
        </button>
        <button
          className={`px-4 py-2 rounded-lg font-semibold ${activeTab === "failed" ? "bg-blue-600 text-white" : "bg-gray-700 hover:bg-gray-600"}`}
        ...
        <table className="table-auto w-full border-collapse border border-gray-700 text-sm md:text-base">
          <thead>
            <tr className="bg-gray-700 text-gray-200">
              ...
              {activeTab === "main" && <th className="border border-gray-700 px-4 py-2">Action</th>}
            </tr>
          </thead>
          <tbody>
            {filteredStudents.length > 0 ? (
              filteredStudents.map((student, index) => (
                <tr key={index} className="text-center hover:bg-gray-700 transition duration-200">
                  <td className="border border-gray-700 px-4 py-2">{student.usn}</td>
                  <td className="border border-gray-700 px-4 py-2">{student.name}</td>
                  <td className="border border-gray-700 px-4 py-2">{student.semester}</td>
                  <td className="border border-gray-700 px-4 py-2">{student.subCode}</td>
                  <td className="border border-gray-700 px-4 py-2">{student.subName}</td>
                  {activeTab === "main" && (
              ...
              <tr>
                <td colSpan={activeTab === "main" ? 6 : 5} className="text-center py-4">
                  {activeTab === "main"
